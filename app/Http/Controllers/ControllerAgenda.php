...
class ControllerAgenda extends Controller
{
    ...
    public function agenda()
    {
        ...
        return view('agenda.agenda')->with('empleados', $empleados);
    }
    ...
    public function reporteagenda()
    {
        $agenda = agenda::withTrashed()
            ->select(['id_agenda', 'seguimiento', 'alias', 'nombre', 'telefono', 'fecha', 'hora', 'deleted_at'])
    ...
    {
        $agenda = agenda::withTrashed()
            ->select('id_agenda', 'seguimiento', 'alias', 'nombre', 'apellido_p', 'apellido_m', 'telefono', 'fecha', 'hora', 'publicidad', 'contesto')
            ->where('id_agenda', $id_agenda)
            ->get();

        return view('agenda.modificaagenda')->with('agenda', $agenda[0]);
    }
    ...
    public function guardacambiosAgenda(Request $request)
    {
        ...
        $agendas->empleado_id = 41;
        $agendas->save();
        Session::flash(
            'mensaje',
            "La agenda $request->nombre
